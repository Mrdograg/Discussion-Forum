...
public class UserActivity {
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		if (user.getUserRole() == UserRole.ADMIN) { // checking if the user is an admin
			questionService.deleteQuestion(question); // deleting a question
		}
          else if (user.getUserRole() == UserRole.MODERATOR) { // checking if the user is an moderator
			if ((question.getUser()==user) ||(question.getUser().getUserRole()==UserRole.USER)) {
		...
		else if (user.getUserRole() == UserRole.USER) { // checking if the user is an regular User
